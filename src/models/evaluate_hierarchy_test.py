...
OUTPUT = ROOT / "models/evaluation/hierarchical_test_metrics.json"
PREDICTIONS = ROOT / "models/evaluation/hierarchical_test_predictions.csv"
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=OUTPUT,
        help="TEST metrics JSON output path",
    )
    parser.add_argument(
        "--predictions",
        type=Path,
        default=PREDICTIONS,
        help="TEST prediction CSV output path",
    )
    parser.add_argument(
        "--use-service-anchors",
        action="store_true",
    ...
    if TEST.name != "test.csv":
        raise ValueError("This evaluator is restricted to held-out TEST")
    if not args.use_service_anchors or not args.use_deterministic_priority:
        raise ValueError(
            "Final TEST evaluation requires frozen service anchors and deterministic priority"
        )
    if args.intent_root.resolve() != (FINAL / "intent").resolve():
        raise ValueError("Final TEST evaluation must use the frozen active intent models")
    frame = pd.read_csv(TEST, dtype=str, keep_default_na=False, encoding="utf-8-sig")
    ...
    service_model_conf = service_conf.copy()
    ...
    trusted_anchor_for_ood = service_anchor.notna() & service_anchor.ne("TAX")
    ood_confidence = service_model_conf.copy()
    ood_confidence.loc[trusted_anchor_for_ood] = 1.0
    ood_decision = ood_confidence.lt(threshold)
    ...
    payload = {
        "evaluation_scope": "held-out TEST only",
        "test_data_used": True,
        "rows": len(frame),
        ...
        "ood_on_in_domain_test": {
            "method": calibration["method"],
            "confidence_threshold": threshold,
            "trusted_anchor_rows": int(trusted_anchor_for_ood.sum()),
    ...
    output = pd.DataFrame(
        {
            ...
            "service_model_confidence": service_model_conf,
            "service_anchor": service_anchor,
            "trusted_anchor_for_ood": trusted_anchor_for_ood,

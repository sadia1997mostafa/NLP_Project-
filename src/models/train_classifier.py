...
def main() -> None:
    ...
    label_map = load_map(args.task)
    if len(label_map) != int(config["num_labels"]):
        raise ValueError("num_labels does not match the frozen label map")
    print(f"Task: {args.task}; labels: {len(label_map)}; model: {config['model_name']}")
    ...
    train = load_dataset(ROOT / "data" / "splits" / "train.csv", args.task, tokenizer, label_map, config["max_length"])
    dev = load_dataset(ROOT / "data" / "splits" / "dev.csv", args.task, tokenizer, label_map, config["max_length"])
